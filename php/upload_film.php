    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $image_target_file = $film_dir . $titre . '_Affiche.' . pathinfo($_FILES["image"]["name"], PATHINFO_EXTENSION);
        ...
        if ($stmt = mysqli_prepare($link, $sql)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                $response = ["success" => true, "message" => "Upload réussi"];
                echo json_encode($response);
            } else {
                $response = ["success" => false, "message" => "Erreur lors de l'insertion du film."];
                echo json_encode($response);
